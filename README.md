...
| H2 콘솔 | http://localhost:8080/h2-console (JDBC URL `jdbc:h2:mem:collab`, 사용자 `sa`, 비밀번호 없음) |

테스트: `./gradlew test` (19개)
...
| POST | `/api/projects/{projectId}/members` | OWNER, ADMIN | 중복 추가 409 |
| PATCH | `/api/projects/{projectId}/members/{userId}` | OWNER, ADMIN | 마지막 OWNER 강등 409 |
| DELETE | `/api/projects/{projectId}/members/{userId}` | OWNER, ADMIN | 마지막 OWNER 제거 409 |
...
| PATCH | `/api/projects/{projectId}/tasks/{taskId}` | 담당자 본인, OWNER, ADMIN | **`version` 필수** |
| DELETE | `/api/projects/{projectId}/tasks/{taskId}` | 담당자 본인, OWNER, ADMIN | 204 |
...
| 상태 | 언제 |
|---|---|
| 400 | 요청 본문 검증 실패 / `X-User-Id` 부재·비수치 |
| 403 | 프로젝트 멤버지만 요구 역할에 못 미침 |
| 404 | 리소스 없음 **또는 프로젝트 비멤버** (아래 설계 결정 참조) |
...
각 항목을 `선택 / 기각한 대안 / 감수한 비용` 세 칸으로 적었습니다. 세 번째 칸이 없으면 장단점을 안 보고 골랐다는 뜻이라고 생각합니다.
...
| **비관적 락(`PESSIMISTIC_WRITE`)** | 트레이드오프 문제가 아니라 **적용 구간이 아닙니다.** 락은 트랜잭션 내에서만 유효한데 이 충돌은 트랜잭션 밖에서 발생합니다. 요구사항을 해결하지 못하면서 처리량만 깎습니다 |
...
**이건 취향이 아니라 명세가 강제한 결론입니다.** 과제가 "프로젝트에 속하지 않은 사용자는 **조회를 포함해** 아무것도 할 수 없다"고 했는데, 403을 주면 "그 id의 프로젝트는 존재한다"가 새어나가 id를 훑는 것만으로 프로젝트 존재 여부를 전수 조사할 수 있습니다.

반대로 이미 경계 안에 있는 멤버에게 404를 주면 "내 프로젝트가 사라졌나?"로 오해합니다. 여기선 403이 정확합니다.

이 규칙은 `ProjectAccessGuard` 한 곳에 있고, 전 도메인이 이 게이트를 통과합니다.
...
**감수한 비용.** 설명할 규칙이 하나 늘었습니다(이 표 2줄). 디버깅 시 구분은 `ProblemDetail`의 `detail` 메시지로 보완합니다.
...
**감수한 비용.** "파라미터"의 문자적 해석과 다릅니다 → 이 문단이 그 근거입니다. Swagger 입력이 번거로워지는 문제는 `OperationCustomizer`(약 10줄)로 `X-User-Id`를 전 오퍼레이션에 자동 주입해 해소했습니다.

부수 효과로 resolver는 **사용자 존재를 검사하지 않습니다.** 없는 id는 `requireMember`에서 멤버십 부재로 자연히 404가 됩니다. 덕분에 resolver에 `UserRepository` 의존이 붙지 않아 단위 테스트가 순수합니다. 다만 `POST /api/projects`는 이 게이트가 덮지 못하는 유일한 진입점이라, 거기서만 사용자 존재를 명시적으로 확인합니다(없으면 404, FK 위반 500이 아니라).

### 4.4 응답 형식 — 성공은 bare DTO, 오류는 RFC 9457

공통 envelope(`ApiResponse<T>`)을 쓰지 않았습니다. HTTP 상태 코드가 이미 성공/실패를 표현하는데 바디의 `"success": true`는 같은 정보의 이중 표현입니다. 오류만 RFC 9457로 두고 성공을 envelope으로 감싸면 응답 형식이 오히려 두 종류가 되어 일관성이 깨집니다.

`spring.mvc.problemdetails.enabled: true` — Spring Boot 3 내장이라 추가 의존성 0입니다. 목록은 Spring `Page`가 그대로 나가 페이징 코드가 0줄이고, springdoc 스키마도 깨끗합니다(제네릭 래핑은 `ApiResponseProjectResponse` 같은 스키마 이름을 만들어냅니다).
...
이 과제의 난이도 핵심은 권한 규칙인데, 그게 전부 `project/` 한 폴더에 응집됩니다. 계층형(`controller/ service/ repository/`)이면 `ProjectService`·`TaskService`·`ProjectAccessGuard`가 각각 다른 폴더로 흩어져, 권한 규칙 한 덩어리를 읽으려면 폴더 4개를 왕복해야 합니다. 도메인이 3개뿐이라 폴더 폭발도 없습니다.
...
프로젝트 경계(`t.project.id = :projectId`)가 **쿼리 안에** 있습니다. 애플리케이션에서 걸러내는 방식과 달리 타 프로젝트 데이터 혼입이 구조적으로 불가능합니다. 목록뿐 아니라 단건 조회도 `findByIdAndProjectId`로 경계를 함께 겁니다.

빈 문자열 `?keyword=`는 서비스에서 `null`로 정규화합니다 — 그대로 두면 `LIKE '%%'`가 되어 필터가 있는 척하게 됩니다.

**감수한 비용.** 조건이 6~7개로 늘면 가독성이 무너집니다. 그 시점이 QueryDSL 도입이 정당해지는 시점이고, 이 과제 범위에서는 오지 않습니다.
...
| H2 인메모리 + `data.sql` | 과제 필수. 체크리스트의 "초기 데이터를 넣어두면 좋습니다"에 대응 |
| Lombok | 보일러플레이트 제거. 사실상 표준이라 설명 비용이 1줄 |
...
커버리지 숫자가 아니라 **재량으로 채운 지점**에 배치했습니다. 명세가 준 부분은 틀리면 명세 탓이지만, 제가 채운 부분은 틀리면 제 탓입니다.

| 파일 | 검증 |
|---|---|
| `ProjectAccessGuardTest` | 비멤버 404 / 역할 부족 403 / **비멤버가 역할 검사를 거쳐도 403이 아니라 404** (검사 순서) |
| `ProjectMemberServiceTest` | 마지막 OWNER 강등·제거 차단, OWNER 2명이면 강등 허용, 중복 멤버 차단 |
| `TaskConcurrencyTest` | 같은 version으로 2회 PATCH → 200 다음 409, **그리고 앞선 변경이 덮이지 않았음** |
| `TaskRepositoryTest` | keyword/status 조합, 페이징 경계, 타 프로젝트 작업 미포함 |
| `CurrentUserArgumentResolverTest` | 헤더 정상 / 부재 / 비수치 |
...
이 서비스는 이미 `project_members`로 애플리케이션 레벨 경계를 강제하고 있습니다. 같은 메커니즘을 한 단계 위(`tenant_id`)로 올리는 것이 가장 작은 변경이고, 규제 요구가 생기면 schema-per-tenant로 승격하는 경로도 열려 있습니다.

### 구체적 변경 지점

1. **진입** — `CurrentUserArgumentResolver`와 같은 자리에서 `X-Tenant-Id`를 해석합니다(인증 도입 시엔 토큰 클레임에서 추출). 인터셉터/필터가 `TenantContext`(ThreadLocal)에 저장하고 **요청 종료 시 반드시 clear** — 스레드 재사용으로 이전 요청의 테넌트가 남으면 그대로 유출입니다
...
3. **제약** — `users.email`의 UNIQUE를 `(tenant_id, email)`로 바꿉니다. 모든 UNIQUE·FK 제약을 테넌트 스코프로 재검토합니다
4. **인덱스** — 조회 인덱스 선두에 `tenant_id`를 둡니다 (`(tenant_id, project_id, status)`)
5. **오류 정책** — **지금의 404/403 규칙이 그대로 확장됩니다.** 크로스 테넌트 접근은 404(존재 은닉), 테넌트 내 권한 위반은 403. 새 규칙을 만들 필요가 없습니다
6. **경계 밖 코드** — 배치·스케줄러·캐시 키에 `tenantId`를 반드시 포함시킵니다. 캐시 키 누락은 테넌트 간 데이터 유출로 직결됩니다
7. **검증** — 테넌트 A 컨텍스트에서 테넌트 B 리소스에 접근하면 404가 나오는 통합 테스트를 회귀 방지선으로 둡니다
