...
**왜.** 과제가 "프로젝트에 속하지 않은 사용자는 **조회를 포함해** 아무것도 할 수 없다"고 했는데, 403을 주면 "그 id의 프로젝트는 존재한다"가 새어나가 id를 훑는 것만으로 프로젝트 존재 여부를 전수 조사할 수 있습니다.
...
**왜.** 이 과제의 난이도 핵심은 권한 규칙인데, 그게 전부 `project/` 한 폴더에 응집됩니다. 계층형(`controller/ service/ repository/`)이면 `ProjectService`·`TaskService`·`ProjectAccessGuard`가 각각 다른 폴더로 흩어져, 권한 규칙 한 덩어리를 읽으려면 폴더 4개를 왕복해야 합니다. 도메인이 3개뿐이라 폴더 폭발도 없습니다.
